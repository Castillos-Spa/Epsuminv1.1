...
import { Contacto } from "@/types/strapi";
import type { BlocksContent } from "@strapi/blocks-react-renderer";

interface SeccionContactoClientProps {
  readonly contacto: Contacto | null;
}

// 🔹 Tipado mínimo para nodos RichText
interface TextChild {
  type: "text";
  text: string;
}
...
// 🔹 Función segura para "aplanar" rich text
const getPlainText = (blocks?: BlocksContent): string => {
  if (!blocks) return "";
  return blocks
    .map((block) =>
      "children" in block
...
export default function SeccionContactoClient({
  contacto,
}: Readonly<SeccionContactoClientProps>) {
  return (
    <section
      className="relative bg-white w-full animate-fade-in"
      id="servicios"
    >
          ...
          <div className="grid md:grid-cols-2 py-10">
            ...
            <div className="pr-4 md:pr-8 flex flex-col justify-center items-start">
              ...
              <p className="text-3xl text-amber-600">
                ¡¡ Estamos aquí para responder a tus preguntas y atender tus
                necesidades !!
              </p>
              <div className="mt-6">
                <Link
